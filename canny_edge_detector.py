...
Obtained from https://github.com/csbanon/canny-edge-detector
"""
...
class CannyEdgeDetector:
  ...
  def gaussian_smoothing(self, I):
    ...
    r = np.linspace(-(self.kernel_size // 2),
  ...
  def hysteresis_threshold(self, I):
    ...
    weak_values_x, weak_values_y = np.where(
        (I <= high_threshold) & (I >= low_threshold))
  ...
  def detect_edges(self):
    ...
    self.I_smooth, self.I_smooth_x, self.I_smooth_y = self.gaussian_smoothing(
        self.I)
    self.I_prime, self.I_prime_x, self.I_prime_y, self.I_theta = self.get_derivatives(
        self.I_smooth_x, self.I_smooth_y)
